/* ============================================================
   ...
   Corrections v6.0 :
   0. col() + normalize() : accès colonnes robuste insensible casse/accents
   1. isExternal : TOUT interne par défaut, externe seulement si mot clé explicite
   2. renderMonthlyTable : suppression double cumul (utilise realCum de buildSCurves)
   3. Filtre honoraires strict sur nom de tâche (contient "honoraire")
   4. renderRessourceTable : tableau honoraires par ressource dans chaque phase
   5. Planning éditable : dates réelles éditables SIA5, tableaux planning SIA2/3/4
   6. Header : upload renommé Transactions + file-status dynamique
   ============================================================ */
...
// ============================================================
// CORRECTION 0 — col() + normalize() : accès colonnes robuste
// ============================================================
...
// ============================================================
// UTILS colonnes spécialisées (utilisent col())
// ============================================================
function getTaskName(row) {
  return String(col(row, 'Nom de la tâche', 'Nom de la tache', 'Tâche', 'Tache', 'Task', 'Nom tache') || 'Inconnu').trim();
}

function getIdProjet(row) {
  return String(col(row, 'ID projet', 'ID Projet', 'id projet', 'ID_projet', 'Projet ID') || '');
}

function getMontant(row) {
  return toNum(col(row, 'Montant total du coût', 'Montant total du cout', 'Montant', 'Coût total', 'Cout total', 'Total cost', 'Montant total'));
}

function getModele(row) {
  return String(col(row, 'Modèle de prévision', 'Modele de prevision', 'Modèle prévision', 'Modele prevision', 'Modele', 'Model') || '');
}

function getDateDebut(row) {
  return toDate(col(row, 'Date de début', 'Date de debut', 'Début', 'Debut', 'Start date', 'Date début'));
}

function getDateFin(row) {
  return toDate(col(row, 'Date de fin', 'Date fin', 'Fin', 'End date', 'Date fin prévue'));
}

function getDateReal(row) {
  return toDate(col(row, 'Date', 'Date du projet', 'date', 'Transaction date', 'Date transaction', 'Date réalisation'));
}

function getNomRessource(row) {
  return String(col(row, 'Nom de la ressource', 'Ressource', 'Resource', 'Nom ressource', 'Nom Ressource', 'Personne', 'Collaborateur') || '').trim();
}

function getTypeRessource(row) {
  return String(col(row, 'Type de ressource', 'Type ressource', 'Type', 'Resource type') || '').toLowerCase();
}

function getCategorie(row) {
  return String(col(row, 'Catégorie', 'Categorie', 'Category', 'Cat') || '').toLowerCase();
}

function getDescription(row) {
  return String(col(row, 'Description', 'Desc', 'description') || '').toLowerCase();
}

function getMetier(row) {
  const task = getTaskName(row).toLowerCase();
  ...
  const raw = getTaskName(row);
  return raw.length > 35 ? raw.substring(0, 33) + '…' : raw;
}

// ============================================================
// CORRECTION 1 — isExternal strict
// TOUT est interne par défaut.
// Externe UNIQUEMENT si "externe" ou "(ext)" dans nom de tâche ou description.
// CPT est toujours interne.
// ============================================================
...
// Filtre honoraires strict : uniquement si "honoraire" dans le nom de tâche
const honorFilter = r => {
  const task = normalize(getTaskName(r));
  return task.includes('honoraire') || task.includes('honoraires');
};

function isHonoraires(row) {
  return honorFilter(row);
}
...
function modelRange(rows, modelName, axis) {
  const filtered = rows.filter(r => getModele(r) === modelName);
  const dates = filtered.flatMap(r => [getDateDebut(r), getDateFin(r)]).filter(Boolean);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // CORRECTION 6 — Upload Prévisions avec file-status
  document.getElementById('prevFile').addEventListener('change', e => {
    const fname = e.target.files[0]?.name || '';
    const statusEl = document.getElementById('prevFileStatus');
    if (statusEl) {
      statusEl.textContent = fname ? '🟢 ' + fname : '⚪ Aucun fichier';
      statusEl.className = 'file-status' + (fname ? ' loaded' : '');
    }
    if (e.target.files[0]) readExcel(e.target.files[0], rows => {
      prevRows = rows;
      const models = [...new Set(rows.map(r => getModele(r)).filter(Boolean))].join(', ');
      document.getElementById('status').textContent = `✅ Prévisions chargées (${rows.length} lignes) — Modèles: ${models}`;
      maybeProcess();
    });
  });

  // CORRECTION 6 — Upload Transactions (ex-Réalisations) avec file-status
  document.getElementById('realFile').addEventListener('change', e => {
    const fname = e.target.files[0]?.name || '';
    const statusEl = document.getElementById('realFileStatus');
    if (statusEl) {
      statusEl.textContent = fname ? '🟢 ' + fname : '⚪ Aucun fichier';
      statusEl.className = 'file-status' + (fname ? ' loaded' : '');
    }
...
function buildSCurves(pRows, rRows) {
  ...
  MODELS.forEach(mn => {
    const rows = pRows.filter(r => getModele(r) === mn);
    const total = rows.reduce((a,r) => a + getMontant(r), 0);
    const dates = rows.flatMap(r => [getDateDebut(r), getDateFin(r)]).filter(Boolean);
    model[mn] = {
      total,
  ...
  const series = {};
  MODELS.forEach(mn => {
    const filtered = pRows.filter(r => getModele(r) === mn);
  ...
  rRows.forEach(r => {
    ...
    if (realMap.has(mm)) realMap.set(mm, realMap.get(mm) + getMontant(r));
    else if (isSOnly && mm < axis[0]) initCum += getMontant(r);
  });
  ...
  charts[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { ticks: { callback: v => toCHF(v), font: { size: 10 } }, grid: { color: '#e8eef5' } }
      }
    }
  });
}
...
// ============================================================
// CORRECTION 2 — TABLEAU MENSUEL : suppression double cumul
// Utilise realCum et realMonth directement depuis buildSCurves
// ============================================================
function renderMonthlyTable(tableId, sData) {
  ...
  // CORRECTION 2 : on utilise directement realCum de buildSCurves — pas de recalcul local
  let html = `<thead><tr>
    <th>Mois</th>
  ...
  axis.forEach((mm, i) => {
    ...
    const reelC = realCum[i] || 0;   // ← directement depuis buildSCurves, pas de recalcul
    const ecM   = reelM - brM;
    const ecC   = reelC - brC;
...
// ============================================================
// CORRECTION 4 — TABLEAU HONORAIRES PAR RESSOURCE
// ============================================================
function renderRessourceTable(tableId, pRows, rRows) {
  const tbl = document.getElementById(tableId);
  if (!tbl) return;

  // Filtre strict honoraires sur nom de tâche
  const hPrev = pRows.filter(honorFilter);
  const hReal = rRows.filter(honorFilter);

  const resMap = {};
  hPrev.forEach(r => {
    const res  = getNomRessource(r) || getTaskName(r);
    const ext  = isExternal(r);
    const key  = `${res}__${ext?'ext':'int'}`;
    const mod  = getModele(r);
    const amt  = getMontant(r);
    if (!resMap[key]) resMap[key] = { res, ext, br:0, at:0, reel:0 };
    if (mod === 'Budget_Rev') resMap[key].br += amt;
    if (mod === 'AT')         resMap[key].at += amt;
  });
  hReal.forEach(r => {
    const res = getNomRessource(r) || getTaskName(r);
    const ext = isExternal(r);
    const key = `${res}__${ext?'ext':'int'}`;
    if (!resMap[key]) resMap[key] = { res, ext, br:0, at:0, reel:0 };
    resMap[key].reel += getMontant(r);
  });

  const items = Object.values(resMap)
    .filter(t => t.br>0 || t.at>0 || t.reel>0)
    .sort((a,b) => (b.at||b.br) - (a.at||a.br));

  const intItems = items.filter(t => !t.ext);
  const extItems = items.filter(t => t.ext);

  let html = `<thead><tr><th>Ressource</th><th>Type (Int/Ext)</th><th>BR</th><th>AT</th><th>Réel</th><th>Écart BR</th><th>Écart AT</th></tr></thead><tbody>`;
  ...
  html += `</tbody><tfoot><tr>
    ...
    <td class="${totE>=0?'negative':'positive'}">${(totE>=0?'+':'')+toCHF(totE)}</td>
  </tr></tfoot>`;
  tbl.innerHTML = html;
}
...
function processRecap() {
  ...
  const summaries = phases.map(ph => {
    const pR = filterPrevByPhase(ph.code);
    const rR = filterRealByPhase(ph.code);
    const budgetRev = pR.filter(r=>getModele(r)==='Budget_Rev').reduce((a,r)=>a+getMontant(r),0);
    const at        = pR.filter(r=>getModele(r)==='AT').reduce((a,r)=>a+getMontant(r),0);
    const reel      = rR.reduce((a,r)=>a+getMontant(r),0);
    const honorPR   = pR.filter(honorFilter);
    const honorRR   = rR.filter(honorFilter);
    const honorBR   = honorPR.filter(r=>getModele(r)==='Budget_Rev').reduce((a,r)=>a+getMontant(r),0);
    const honorAT   = honorPR.filter(r=>getModele(r)==='AT').reduce((a,r)=>a+getMontant(r),0);
    const honorReel = honorRR.reduce((a,r)=>a+getMontant(r),0);
    const allDates  = [...pR,...rR].flatMap(r=>[getDateDebut(r),getDateFin(r),getDateReal(r)]).filter(Boolean);
    return {
      ...ph,
      budgetRev, at, reel,
      ecartBR: reel-budgetRev, ecartAT: reel-at,
      honorBR, honorAT, honorReel,
      planStart: allDates.length?new Date(Math.min(...allDates)):null,
      planEnd:   allDates.length?new Date(Math.max(...allDates)):null
    };
  });
  ...
  if(rcCtx){
    const nz=summaries.filter(s=>s.budgetRev>0||s.at>0||s.reel>0);
    if(nz.length) charts['recapBudgetChart']=new Chart(rcCtx,{
      type:'bar', data:{ labels:nz.map(s=>s.name),
        datasets:[
          {label:'Budget Révisé',data:nz.map(s=>s.budgetRev),backgroundColor:GE_ORANGE+'cc'},
          {label:'AT',data:nz.map(s=>s.at),backgroundColor:GE_BLUE2+'cc'},
          {label:'Réel',data:nz.map(s=>s.reel),backgroundColor:GE_RED+'cc'}
        ]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'}},scales:{y:{ticks:{callback:v=>toCHF(v)}}}}
    });
  }
  ...
  if(tbl){
    ...
    summaries.forEach(s=>{
      ...
      html+=`<tr><td><strong>${s.name}</strong></td><td>${toCHF(s.budgetRev)}</td><td>${toCHF(s.at)}</td><td>${toCHF(s.reel)}</td>
    ...
    html+=`</tbody><tfoot><tr><td>TOTAL</td><td>${toCHF(totalBR)}</td><td>${toCHF(totalAT)}</td><td>${toCHF(totalReel)}</td>
      ...
      <td>${totalAT>0?(totalReel/totalAT*100).toFixed(1):0}%</td></tr></tfoot>`;
    tbl.innerHTML=html;
  }
  ...
  if(riCtx){
    const nzH=summaries.filter(s=>s.honorBR>0||s.honorAT>0||s.honorReel>0);
    if(nzH.length) charts['recapImputChart']=new Chart(riCtx,{
      type:'bar', data:{ labels:nzH.map(s=>s.name),
        datasets:[
          {label:'Honoraires BR',data:nzH.map(s=>s.honorBR),backgroundColor:GE_ORANGE+'cc'},
          {label:'Honoraires AT',data:nzH.map(s=>s.honorAT),backgroundColor:GE_BLUE2+'cc'},
          {label:'Réel',data:nzH.map(s=>s.honorReel),backgroundColor:GE_GREEN+'cc'}
        ]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'}},scales:{y:{ticks:{callback:v=>toCHF(v)}}}}
    });
  }
  ...
  if(tblI){
    ...
    summaries.forEach(s=>{ const ec=s.honorReel-s.honorAT;
  ...
  if(ganttCtx){
    const vp=summaries.filter(s=>s.planStart&&s.planEnd);
    if(vp.length){
      const allStart=new Date(Math.min(...vp.map(s=>s.planStart)));
      const allEnd=new Date(Math.max(...vp.map(s=>s.planEnd)));
      ...
      const bars=vp.map(s=>{
        ...
        return{label:s.name,base:si>=0?si:0,len:ei>=si?ei-si+1:1,color:s.color};
      });
      charts['recapPlanningChart']=new Chart(ganttCtx,{
        type:'bar',
        data:{ labels:vp.map(s=>s.name),
          datasets:[
            {label:'Début',data:bars.map(b=>b.base),backgroundColor:'transparent',borderWidth:0},
            {label:'Durée',data:bars.map(b=>b.len),backgroundColor:vp.map(s=>s.color+'cc'),borderRadius:4}
          ]},
        options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
          plugins:{legend:{display:false},todayLine:{label:yyyymm(new Date())}},
          scales:{x:{stacked:true,ticks:{callback:v=>axis[v]?fmtMonth(axis[v]):''}},y:{stacked:true}}}
      });
      ganttCtx.parentElement.style.height=`${Math.max(200,vp.length*80)}px`;
    }
  }
}
...
function processPhase(phaseName, phaseCode) {
  ...
  const budgetRev=pRows.filter(r=>getModele(r)==='Budget_Rev').reduce((a,r)=>a+getMontant(r),0);
  const at=pRows.filter(r=>getModele(r)==='AT').reduce((a,r)=>a+getMontant(r),0);
  const reel=rRows.reduce((a,r)=>a+getMontant(r),0);
  const ecart=reel-at;
  ...
  const taskMap={};
  pRows.forEach(r=>{
    const task=getTaskName(r), model=getModele(r), amount=getMontant(r);
  ...
  rRows.forEach(r=>{
    ...
    if(!taskMap[task]) taskMap[task]={task,budget:0,budgetRev:0,at:0,reel:0};
    taskMap[task].reel+=getMontant(r);
  });
  ...
  if(tblBudget){
    ...
    taskList.forEach(t=>{
      ...
      html+=`<tr><td><strong>${t.task}</strong></td><td>${toCHF(t.budget)}</td><td>${toCHF(t.budgetRev)}</td><td>${toCHF(t.at)}</td><td>${toCHF(t.reel)}</td>
        <td class="${eBR>=0?'negative':'positive'}">${eBR!==0?(eBR>=0?'+':'')+toCHF(eBR):'—'}</td>
        <td class="${eAT>=0?'negative':'positive'}">${eAT!==0?(eAT>=0?'+':'')+toCHF(eAT):'—'}</td>
        <td>${pAT}%</td></tr>`;
    });
    html+=`</tbody><tfoot><tr><td>TOTAL</td><td>${toCHF(taskList.reduce((a,t)=>a+t.budget,0))}</td>
  ...
  if(pieBRCtx){
    const vt=taskList.filter(t=>t.budgetRev>0);
    if(vt.length) charts[pieBRId]=new Chart(pieBRCtx,{
      type:'doughnut',
      data:{labels:vt.map(t=>t.task.length>22?t.task.substring(0,20)+'…':t.task),
        datasets:[{data:vt.map(t=>t.budgetRev),backgroundColor:vt.map((_,i)=>PIE_COLORS[i%PIE_COLORS.length]),borderWidth:2,borderColor:'#fff'}]},
  ...
  if(barCtx){
    const vb=taskList.filter(t=>t.at>0||t.reel>0);
    if(vb.length) charts[barId]=new Chart(barCtx,{
      type:'bar',
      data:{labels:vb.map(t=>t.task.length>20?t.task.substring(0,18)+'…':t.task),
        datasets:[
          {label:'AT',data:vb.map(t=>t.at),backgroundColor:GE_BLUE2+'cc'},
          {label:'Réel',data:vb.map(t=>t.reel),backgroundColor:GE_RED+'cc'}
        ]},
      options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
        plugins:{legend:{position:'top'}},scales:{x:{ticks:{callback:v=>toCHF(v)}}}}
    });
  }
...
// ============================================================
// CORRECTION 3 — IMPUTATIONS PAR PHASE (SIA2/3/4)
// Filtre strict : uniquement lignes dont nom de tâche contient "honoraire"
// ============================================================
function processImputationsPhase(phaseName, phaseCode, pRows, rRows) {
  const phaseNum=phaseCode.replace('0','');

  // CORRECTION 3 : filtre strict sur nom de tâche
  const honorPrev=pRows.filter(honorFilter);
  const honorReal=rRows.filter(honorFilter);

  const taskMap={};
  honorPrev.forEach(r=>{
    const task=getTaskName(r), model=getModele(r), ext=isExternal(r);
    const key=`${task}__${ext?'ext':'int'}`;
    if(!taskMap[key]) taskMap[key]={task,ext,br:0,at:0,reel:0};
    if(model==='Budget_Rev') taskMap[key].br+=getMontant(r);
    if(model==='AT')         taskMap[key].at+=getMontant(r);
  });
  honorReal.forEach(r=>{
    const task=getTaskName(r),ext=isExternal(r);
    const key=`${task}__${ext?'ext':'int'}`;
    if(!taskMap[key]) taskMap[key]={task,ext,br:0,at:0,reel:0};
    taskMap[key].reel+=getMontant(r);
  });
  ...
  if(tbl){
    ...
    const renderSection=(list,type,cls,clsSub)=>{
      ...
      const sE=sReel-sAT;
      rows+=`<tr class="${clsSub}"><td colspan="2"><strong>Sous-total ${type==='int'?'Interne':'Externe'}</strong></td>
        <td>${toCHF(sBR)}</td><td>${toCHF(sAT)}</td><td>${toCHF(sReel)}</td>
        <td class="${(sReel-sBR)>=0?'negative':'positive'}">${((sReel-sBR)>=0?'+':'')+toCHF(sReel-sBR)}</td>
        <td class="${sE>=0?'negative':'positive'}">${(sE>=0?'+':'')+toCHF(sE)}</td></tr>`;
      return rows;
    };
    html+=renderSection(intItems,'int','section-header-int','subtotal-int');
    html+=renderSection(extItems,'ext','section-header-ext','subtotal-ext');
    html+=`</tbody><tfoot><tr><td colspan="2"><strong>TOTAL HONORAIRES</strong></td>
      <td>${toCHF(totBR)}</td><td>${toCHF(totAT)}</td><td>${toCHF(totReel)}</td>
      <td class="${(totReel-totBR)>=0?'negative':'positive'}">${((totReel-totBR)>=0?'+':'')+toCHF(totReel-totBR)}</td>
      <td class="${ecartAT>=0?'negative':'positive'}">${(ecartAT>=0?'+':'')+toCHF(ecartAT)}</td></tr></tfoot>`;
    tbl.innerHTML=html;
  }

  // CORRECTION 4 — Tableau Honoraires par Ressource
  renderRessourceTable(`tableRessourceSIA${phaseNum}`, pRows, rRows);
}

// ============================================================
// CORRECTION 5 — PLANNING PHASES SIA 2/3/4
// Ajoute un tableau avec dates réelles éditables sous chaque Gantt
// ============================================================
function processPlanningPhase(phaseName, phaseCode, pRows, rRows) {
  ...
  const ganttData=taskNames.map(task=>{
    ...
    const getBR=taskPrev.filter(r=>getModele(r)==='Budget_Rev');
    const getAT=taskPrev.filter(r=>getModele(r)==='AT');
    const realT=rRows.filter(r=>getTaskName(r)===task);
    const getDates=rows=>{
      const dates=rows.flatMap(r=>[getDateDebut(r),getDateFin(r),getDateReal(r)]).filter(Boolean);
      return dates.length?{start:new Date(Math.min(...dates)),end:new Date(Math.max(...dates))}:null;
    };
    return{task,br:getDates(getBR),at:getDates(getAT),reel:getDates(realT)};
  ...
  const dateToIdx=d=>{ if(!d) return -1; const mm=yyyymm(firstOfMonth(d)); const i=axis.indexOf(mm); return i>=0?i:0; };
  const buildBar=dateObj=>{ if(!dateObj) return[null,null]; const si=dateToIdx(dateObj.start),ei=dateToIdx(dateObj.end); return[si,ei-si+1]; };

  charts[ganttId]=new Chart(ctx,{
    type:'bar',
    data:{ labels:ganttData.map(t=>t.task.length>22?t.task.substring(0,22)+'…':t.task),
      datasets:[
        {label:'BR offset',data:ganttData.map(t=>buildBar(t.br)[0]),backgroundColor:'transparent',borderWidth:0,stack:'br'},
        {label:'Budget Révisé',data:ganttData.map(t=>buildBar(t.br)[1]),backgroundColor:GE_ORANGE+'cc',borderRadius:3,barThickness:12,stack:'br'},
        {label:'AT offset',data:ganttData.map(t=>buildBar(t.at)[0]),backgroundColor:'transparent',borderWidth:0,stack:'at'},
        {label:'AT',data:ganttData.map(t=>buildBar(t.at)[1]),backgroundColor:GE_BLUE2+'cc',borderRadius:3,barThickness:12,stack:'at'},
        {label:'Réel offset',data:ganttData.map(t=>buildBar(t.reel)[0]),backgroundColor:'transparent',borderWidth:0,stack:'reel'},
        {label:'Réel',data:ganttData.map(t=>buildBar(t.reel)[1]),backgroundColor:GE_RED+'cc',borderRadius:3,barThickness:12,stack:'reel'}
      ]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
      plugins:{legend:{labels:{filter:item=>!item.text.includes('offset'),usePointStyle:true,font:{size:11}}},todayLine:{label:yyyymm(new Date())}},
      scales:{x:{stacked:true,ticks:{callback:v=>axis[v]?fmtMonth(axis[v]):''},font:{size:10}},y:{stacked:true}}}
  });
  ctx.parentElement.style.height=`${Math.max(250,ganttData.length*55+80)}px`;

  // CORRECTION 5 — Tableau planning avec dates réelles éditables
  const tblPlan=document.getElementById(`tablePlanning${phaseName}`);
  if(!tblPlan) return;
  ...
  ganttData.forEach(t=>{
    const manKey=`${phaseName}_${t.task}`;
    const manual=manualPlanningDates[manKey]||{};
    const brS=t.br?.start?toDateInput(t.br.start):'—';
    const brE=t.br?.end?toDateInput(t.br.end):'—';
    const atS=t.at?.start?toDateInput(t.at.start):'—';
    const atE=t.at?.end?toDateInput(t.at.end):'—';
    const reelStart=manual.reelStart||(t.reel?.start)||null;
    const reelEnd=manual.reelEnd||(t.reel?.end)||null;

    let statut='',statusClass='';
    if(reelStart&&reelEnd&&reelEnd<today){statut='Terminé';statusClass='termine';}
    else if(reelStart&&reelStart<=today){
      if(t.at?.end&&today>t.at.end){statut='Retard';statusClass='retard';}
      else{statut='En cours';statusClass='en-cours';}
    }else{statut='Prévu';statusClass='prevu';}

    html+=`<tr>
      <td><strong>${t.task}</strong></td>
      <td><span class="readonly-date">${brS}</span></td>
      <td><span class="readonly-date">${brE}</span></td>
      <td><span class="readonly-date">${atS}</span></td>
      <td><span class="readonly-date">${atE}</span></td>
      <td><input type="date" class="date-input plan-input-phase" data-phase="${phaseName}" data-task="${encodeURIComponent(t.task)}" data-field="reelStart" value="${toDateInput(reelStart)}"></td>
      <td><input type="date" class="date-input plan-input-phase" data-phase="${phaseName}" data-task="${encodeURIComponent(t.task)}" data-field="reelEnd" value="${toDateInput(reelEnd)}"></td>
  ...
  tblPlan.querySelectorAll('.plan-input-phase').forEach(input=>{
    input.addEventListener('change',()=>{
      const phase=input.dataset.phase;
      const task=decodeURIComponent(input.dataset.task);
      const field=input.dataset.field;
      const manK=`${phase}_${task}`;
      if(!manualPlanningDates[manK]) manualPlanningDates[manK]={};
      manualPlanningDates[manK][field]=input.value?new Date(input.value):null;
      // Recalcul Gantt
      const pR=filterPrevByPhase(phaseCode);
      const rR=filterRealByPhase(phaseCode);
      processPlanningPhase(phase,phaseCode,pR,rR);
    });
  });
}
...
function processSIA5Budget() {
  ...
  if(tblBudget){
    ...
    pRows.forEach(r=>{
      ...
      const model=getModele(r),amt=getMontant(r);
    ...
    Object.entries(taskMap).forEach(([id,t])=>{
      if(!t.budget&&!t.budgetRev&&!t.at&&!t.reel) return;
      const eBR=t.reel-t.budgetRev,eAT=t.reel-t.at;
      const pAT=t.at>0?(t.reel/t.at*100).toFixed(1):'—';
      html+=`<tr><td><strong>${getPN(id)}</strong></td>
        <td>${toCHF(t.budget)}</td><td>${toCHF(t.budgetRev)}</td><td>${toCHF(t.at)}</td><td>${toCHF(t.reel)}</td>
        <td class="${eBR>=0?'negative':'positive'}">${(eBR>=0?'+':'')+toCHF(eBR)}</td>
        <td class="${eAT>=0?'negative':'positive'}">${(eAT>=0?'+':'')+toCHF(eAT)}</td>
        <td>${pAT}%</td></tr>`;
    });
    const bTotal=pRows.filter(r=>getModele(r)==='Budget').reduce((a,r)=>a+getMontant(r),0);
    html+=`</tbody><tfoot><tr><td>TOTAL</td><td>${toCHF(bTotal)}</td><td>${toCHF(budgetRev)}</td><td>${toCHF(at)}</td><td>${toCHF(reel)}</td>
      <td class="${(reel-budgetRev)>=0?'negative':'positive'}">${((reel-budgetRev)>=0?'+':'')+toCHF(reel-budgetRev)}</td>
      <td class="${ecart>=0?'negative':'positive'}">${(ecart>=0?'+':'')+toCHF(ecart)}</td>
      <td>${pct}%</td></tr></tfoot>`;
    tblBudget.innerHTML=html;
  ...
  if(pieSIA5Ctx){
    const pieData=PROJECT_ORDER.map(id=>({
      label:getPN(id),
      value:pRows.filter(r=>getIdProjet(r)===id&&getModele(r)==='Budget_Rev').reduce((a,r)=>a+getMontant(r),0)
    })).filter(d=>d.value>0);
    if(pieData.length) charts['pieSIA5BR']=new Chart(pieSIA5Ctx,{
      type:'doughnut',
      data:{labels:pieData.map(d=>d.label),datasets:[{data:pieData.map(d=>d.value),backgroundColor:pieData.map((_,i)=>PIE_COLORS[i%PIE_COLORS.length]),borderWidth:2,borderColor:'#fff'}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{size:11},usePointStyle:true}},tooltip:{callbacks:{label:ctx=>` ${ctx.label}: CHF ${toCHF(ctx.parsed)}`}}}}
    });
  }
  ...
  if(barSIA5Ctx){
    const barData=PROJECT_ORDER.map(id=>{
      const atV=pRows.filter(r=>getIdProjet(r)===id&&getModele(r)==='AT').reduce((a,r)=>a+getMontant(r),0);
      const reelV=rRows.filter(r=>getIdProjet(r)===id).reduce((a,r)=>a+getMontant(r),0);
      return{label:getPN(id),at:atV,reel:reelV};
    }).filter(d=>d.at>0||d.reel>0);
    if(barData.length) charts['barSIA5']=new Chart(barSIA5Ctx,{
      type:'bar',data:{labels:barData.map(d=>d.label),datasets:[
      ...
      options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'}},scales:{x:{ticks:{callback:v=>toCHF(v)}}}}
    });
  }
}

// ============================================================
// CORRECTION 3+4 — SIA 5 IMPUTATIONS
// Filtre strict sur nom de tâche "honoraire" + tableau ressources
// ============================================================
function processSIA5Imputations() {
  ...
  const metierMap={};
  honorPrev.forEach(r=>{
    const metier=getMetier(r), model=getModele(r), ext=isExternal(r);
    const key=`${metier}__${ext?'ext':'int'}`;
    if(!metierMap[key]) metierMap[key]={metier,ext,br:0,at:0,reel:0};
    if(model==='Budget_Rev') metierMap[key].br+=getMontant(r);
    if(model==='AT')         metierMap[key].at+=getMontant(r);
  });
  honorReal.forEach(r=>{
    const metier=getMetier(r),ext=isExternal(r);
    const key=`${metier}__${ext?'ext':'int'}`;
    if(!metierMap[key]) metierMap[key]={metier,ext,br:0,at:0,reel:0};
    metierMap[key].reel+=getMontant(r);
  });
  ...
  if(barImputCtx&&items.length){
    ...
    charts['barImputSIA5']=new Chart(barImputCtx,{
      type:'bar',
      data:{labels:sorted.map(t=>`${t.metier}${t.ext?' (ext)':''}`),
        datasets:[
      ...
      options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
        plugins:{legend:{position:'top',labels:{usePointStyle:true,font:{size:11}}},tooltip:{callbacks:{label:ctx=>` ${ctx.dataset.label}: CHF ${toCHF(ctx.parsed.x)}`}}},
        scales:{x:{ticks:{callback:v=>toCHF(v),font:{size:10}},grid:{color:'#e8eef5'}},y:{ticks:{font:{size:11}}}}}
    });
    barImputCtx.parentElement.style.height=`${Math.max(200,sorted.length*50+80)}px`;
  }

  // CORRECTION 4 — Tableau Honoraires par Ressource SIA5
  renderRessourceTable('tableRessourceSIA5', pRows, rRows);
}

// ============================================================
// CORRECTION 5 — SIA 5 PLANNING
// BR et AT = lecture seule (span)
// Réel Début / Réel Fin = éditables (input type=date)
// ============================================================
function processPlanningSIA5() {
  ...
  const planData=PROJECT_ORDER.map(id=>{
    const brRows=pRows.filter(r=>getIdProjet(r)===id&&getModele(r)==='Budget_Rev');
    const atRows=pRows.filter(r=>getIdProjet(r)===id&&getModele(r)==='AT');
    const reelRows=rRows.filter(r=>getIdProjet(r)===id);
    const getDates=rows=>{
      const dates=rows.flatMap(r=>[getDateDebut(r),getDateFin(r),getDateReal(r)]).filter(Boolean);
      return dates.length?{start:new Date(Math.min(...dates)),end:new Date(Math.max(...dates))}:null;
    };
    ...
    return{
      ...
      // CORRECTION 5 : Réel éditable, BR/AT lecture seule
  ...
  if(tblPlan){
    ...
    planData.forEach(p=>{
      const today=new Date();
      let statut='',statusClass='';
      if(p.reel.start&&p.reel.end&&p.reel.end<today){statut='Terminé';statusClass='termine';}
      ...
      html+=`<tr>
        ...
        <!-- BR et AT : lecture seule -->
        <td><span class="readonly-date">${p.br.start?toDateInput(p.br.start):'<em style="color:#bbb">—</em>'}</span></td>
        <td><span class="readonly-date">${p.br.end  ?toDateInput(p.br.end)  :'<em style="color:#bbb">—</em>'}</span></td>
        <td><span class="readonly-date">${p.at.start?toDateInput(p.at.start):'<em style="color:#bbb">—</em>'}</span></td>
        <td><span class="readonly-date">${p.at.end  ?toDateInput(p.at.end)  :'<em style="color:#bbb">—</em>'}</span></td>
        <!-- Réel : éditables -->
    ...
    tblPlan.querySelectorAll('.plan-input').forEach(input=>{
      input.addEventListener('change',()=>{
        ...
        const updatedPlan=PROJECT_ORDER.map(id=>{
          const manual=manualPlanningDates[id]||{};
          const orig=planData.find(p=>p.id===id)||{};
          return{...orig,reel:{start:manual.reelStart||orig.reel?.start||null,end:manual.reelEnd||orig.reel?.end||null}};
        });
        renderGanttSIA5(updatedPlan);
      });
    });
  }
...
function collectManualDatesSIA5() {
  document.querySelectorAll('#tablePlanningSIA5 .plan-input').forEach(input=>{
...
function renderGanttSIA5(planData) {
  ...
  const axisStart=new Date(Math.min(...allDates)),axisEnd=new Date(Math.max(...allDates));
  const axis=buildAxis(axisStart,axisEnd);
  const toIdx=d=>{ if(!d) return null; const mm=yyyymm(firstOfMonth(d)); const i=axis.indexOf(mm); return i>=0?i:null; };
  const buildBar=(s,e)=>{ const si=toIdx(s),ei=toIdx(e); if(si===null) return[null,null]; return[si,ei!==null?Math.max(1,ei-si+1):1]; };
  charts['ganttSIA5']=new Chart(ctx,{
    type:'bar',
    data:{labels:valid.map(p=>p.name),
      datasets:[
        {label:'BR off',data:valid.map(p=>buildBar(p.br.start,p.br.end)[0]),backgroundColor:'transparent',borderWidth:0,stack:'br',barThickness:18},
        {label:'BR',data:valid.map(p=>buildBar(p.br.start,p.br.end)[1]),backgroundColor:GE_ORANGE+'cc',borderRadius:3,stack:'br',barThickness:18},
        {label:'AT off',data:valid.map(p=>buildBar(p.at.start,p.at.end)[0]),backgroundColor:'transparent',borderWidth:0,stack:'at',barThickness:18},
        {label:'AT',data:valid.map(p=>buildBar(p.at.start,p.at.end)[1]),backgroundColor:GE_BLUE2+'cc',borderRadius:3,stack:'at',barThickness:18},
        {label:'Réel off',data:valid.map(p=>buildBar(p.reel.start,p.reel.end)[0]),backgroundColor:'transparent',borderWidth:0,stack:'reel',barThickness:18},
        {label:'Réel',data:valid.map(p=>buildBar(p.reel.start,p.reel.end)[1]),backgroundColor:GE_RED+'cc',borderRadius:3,stack:'reel',barThickness:18}
      ]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
      plugins:{legend:{labels:{filter:item=>!item.text.includes('off'),usePointStyle:true,font:{size:12}}},todayLine:{label:yyyymm(new Date())}},
      scales:{x:{stacked:true,ticks:{callback:v=>axis[v]?fmtMonth(axis[v]):'',font:{size:10}},grid:{color:'#e8eef5'}},y:{stacked:true,ticks:{font:{size:11}}}}}
  });
...
function exportChartPNG(canvasId) {
  ...
  canvas.toBlob(blob=>{
    ...
    a.href=url; a.download=`${canvasId}_${new Date().toISOString().slice(0,10)}.png`;
...
async function generatePDF() {
  ...
  const pageW=210,pageH=297,margin=12,contentW=210-24;
  let y=margin,pageCount=1;

  const checkNewPage=(needed=20)=>{
    if(y+needed>pageH-margin){doc.addPage();pageCount++;y=margin;renderHeader();}
  };
  const renderHeader=()=>{
    doc.setFillColor(22,58,95);doc.rect(0,0,pageW,18,'F');
  ...
  const addTitle=(text,level=1)=>{
    checkNewPage(14);
    if(level===1){
      doc.setFillColor(22,58,95);doc.rect(margin,y,contentW,9,'F');
      doc.setTextColor(255,255,255);doc.setFontSize(10);doc.setFont('helvetica','bold');
      doc.text(text,margin+3,y+6.5);doc.setTextColor(0,0,0);y+=13;
    }else{
      doc.setFillColor(232,135,58);doc.rect(margin,y,3,7,'F');
      doc.setFontSize(9);doc.setFont('helvetica','bold');doc.setTextColor(22,58,95);
      doc.text(text,margin+5,y+5.5);doc.setTextColor(0,0,0);y+=11;
    }
  };
  const addKPIs=(kpiSectionId)=>{
    ...
    cards.forEach((card,i)=>{
      ...
      doc.setFillColor(240,244,249);doc.setDrawColor(200,210,220);
      doc.roundedRect(x+1,y,colW-2,16,2,2,'FD');
      doc.setFontSize(6.5);doc.setFont('helvetica','bold');doc.setTextColor(90,120,155);
      doc.text(label.toUpperCase(),x+colW/2,y+5,{align:'center'});
      doc.setFontSize(10);doc.setTextColor(22,58,95);
      doc.text(value,x+colW/2,y+11,{align:'center'});
      doc.setFontSize(6);doc.setTextColor(130,150,170);
      doc.text(unit,x+colW/2,y+15,{align:'center'});
  ...
  const addTable=(tableId,maxRows=40)=>{
    ...
    checkNewPage(12);
    doc.setFillColor(22,58,95);doc.rect(margin,y,contentW,7,'F');
    ...
    rows.forEach((row,ri)=>{
      ...
      if(isFooter){doc.setFillColor(220,230,240);doc.rect(margin,y,contentW,6.5,'F');doc.setFont('helvetica','bold');}
      else{if(ri%2===0){doc.setFillColor(248,251,255);doc.rect(margin,y,contentW,6.5,'F');}doc.setFont('helvetica','normal');}
      doc.setTextColor(0,0,0);doc.setFontSize(6.2);
  ...
  const addChartCanvas=async(canvasId,heightMM=60)=>{
    ...
    try{
      ...
      doc.addImage(imgData,'PNG',margin,y,contentW,heightMM);y+=heightMM+5;
  ...
  const renderPhasePDF=async(phaseLabel,phaseSuffix)=>{
    ...
    addTitle('Tableau Mensuel',2);addTable(`table${phaseSuffix}Monthly`,24);
    addTitle('Tableau Budgets',2);addTable(`table${phaseSuffix}Budget`);
    ...
    addTable(`tablePlanning${phaseSuffix}`);
  };

  renderHeader();
  doc.setFillColor(22,58,95);doc.rect(margin,y,contentW,28,'F');
  doc.setTextColor(255,255,255);doc.setFontSize(16);doc.setFont('helvetica','bold');
  doc.text('Dashboard CAD — Groupe E Celsius',pageW/2,y+12,{align:'center'});
  doc.setFontSize(10);doc.setFont('helvetica','normal');
  const scopeLabel={recap:'Récapitulatif Global',sia2:'Phase SIA 2',sia3:'Phase SIA 3',sia4:'Phase SIA 4',sia5:'Phase SIA 5',all:'Rapport Complet'}[scope]||scope;
  ...
  if(scope==='sia5'||scope==='all'){
    ...
    addTitle('Tableau Mensuel',2);addTable('tableSIA5Monthly',24);
    addTitle('Tableau Budgets',2);addTable('tableSIA5Budget');
    ...
    addTable('tableRessourceSIA5',30);
  ...
  for(let i=1;i<=totalPages;i++){
    doc.setPage(i);doc.setFontSize(7.5);doc.setTextColor(120,140,160);doc.setFont('helvetica','normal');
    doc.text(`Groupe E Celsius — Dashboard CAD CL150096 — Page ${i}/${totalPages}`,pageW/2,pageH-5,{align:'center'});
    doc.setDrawColor(200,210,220);doc.line(margin,pageH-8,pageW-margin,pageH-8);
  }
